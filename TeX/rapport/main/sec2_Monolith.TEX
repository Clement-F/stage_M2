...
\begin{equation}
    u_h^i = \sum_{m=1}^{N_S^i}\overline{u}_m^i \phi_m^i,\qquad \overline{u}_m^i = \frac{1}{|S_m^i|} \int_{S^i_m} u_h^i dx
\end{equation}
...
\end{equation}
On fera attention que la seconde intégrale n'est exacte que si l'ont utilise une méthode de quadrature sur $S_m^i$ et non sur $\omega_i$.
...
Dans le cas échéant ($N_s > N_k$), on utilisera un opérateur de reconstruction $\mathcal{R} = \left(\mathcal{P}^t\,\mathcal{P}\right)^{-1} \mathcal{P}^t$  
...
Comme cette équation est vraie pour tout polynôme de $\mathbb{P}^k(\omega_i)$, elle est en particulier vrai pour $\phi_m^i$. Cela nous amène presque à un flux ne dépendant que de $S_m^i$ : 
\begin{equation}
...
On écrira, par soucis de clarté  $\widehat{{F}_{mp}}$ le flux allant de la sous-cellule $S_m^i$ à la sous-cellule $S_p^j$ pour $j\in \{i-1,i,i+1\}$ 
...
Il n'est cependant pas impossible d'imposer un tel critère, en suivant une preuve inspiré de  \cite{VILAR2025}  \cite{Rueda-Ramírez2024} \cite{Kuzmin2020}. En supposant $\rho >0$, on cherche un critère pour obtenir $\rho \varepsilon =  \rho \,\rho e - \frac{|\rho v|^2}{2} >0$.\\
...
Cette condition garantie à la fois que $\widetilde{\rho}^{\pm} >0$ mais aussi que $\widetilde{\varepsilon}^{\pm}>0$ 
...
\noindent
La première méthode consiste à comparer deux écriture de la dérivée d'un polynôme dans la base polynômiale : 
...
$$ G_h K = G_h' M$$
Il est alors possible de récupérer les coordonnées $G_h'$ en inversant la matrice de massse :$G_h' = M^{-1}G_h K$.\\
À partir des coordonées de la dérivée du polynôme, il est possible d'obtenir sa moyenne par un passage similaire à \eqref{passage_svf}.
