...
$A(\mathbf{u})$ est diagonalisable en $M$ valeurs propres réelles bien ordonnée $\lambda_1(\mathbf{u})\leq \lambda_2(\mathbf{u}) \leq \dots \lambda_m(\mathbf{u})$ 
...
Il a été prouvé \cite{godlewski2021numerical} que si \eqref{system_hyperbolique} admet une pair d'entropie ($\eta,\phi$), alors le système est symétrisable($\exists A_0\in \mathcal{M}_{M\times M}$ telle que $A_0\, A$ est symétrique)
...
\noindent
Une remarque importante est que même si la projection $\mathbb{L}^2$ est égale en produit scalaire ($\int_{\omega_i} {u}_0 \sigma_q dx = \int_{\omega_i} {u}_{h,0} \sigma_q dx$ ), elle n'est pas égale en tout point.
