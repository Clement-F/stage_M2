...
Soit $\Omega\subset \R^M$ l'espace des phases, $\mathbb{D} =]a,b[ \subset \R$ le domaine de calcul et $\mathbf{f} : \Omega \to \R^M$ une fonction régulière. 
Soit $\mathbf{u} : \mathbb{D} \times \R^+ \to \Omega$ la solution du problème : 
...
\noindent
On dit alors que le système \eqref{system_hyperbolique} est \emph{hyperbolique} si et seulement si : 
...
et si leurs vecteurs propres associés sont linéairement indépendants.
Si les inégalités sont strictes, alors le système \eqref{system_hyperbolique} est dit \emph{strictement hyperbolique}.

\paragraph{Solutions faibles \\}

En multipliant \eqref{system_hyperbolique} par une fonction test $\phi \in \mathcal{D}(\R)$ puis en intégrant et en passant par une intégration par partie,
on obtient une nouvelle formulation du problème \eqref{system_hyperbolique}. 
La solution $\mathbf{u}$ de ce problème est alors moins exigente en régularité
...
On dit que la fonction $\mathbf{u}$ est une \emph{solution faible} du problème \eqref{system_hyperbolique} si elle est solution de l'équation \eqref{Formulation_Faible}.\\
Remarque : si $\mathbf{u}$ est solution faible et $\mathbf{u}\in \mathcal{C}^1$, elle est aussi solution forte \red{preuve (si place)}. i.e solution de \eqref{system_hyperbolique}.\\

Nous chercherons des solutions bornées (i.e dans $L^\infty$) et à variations bornées (i.e dans $BV$ 
\footnote{Soit $[a,b]\subset \R,\, f:[a,b]\to \R$, On donne $V^a_b(f)=\sup_{P\in \mathbf{P}} \sum_i^{n_p-1}|f(x_{i+1})-f(x_i)|$, 
...
Soit $\eta : \Omega \to \R$ une fonction convexe, on dit qu'elle est une \emph{entropie} pour le système \eqref{system_hyperbolique},
s'il existe une fonction flux $\phi : \Omega \to \R$ telle que :
\begin{equation}
    \nabla \eta(u) A(u) = \nabla \phi (u) \qquad  \forall u \in \Omega
\end{equation}
On note que pour le cas scalaire, toute fonction convexe $\eta$ est entropie du système.\\
Il a été prouvé (voir \cite{godlewski2021numerical}) que si \eqref{system_hyperbolique} admet une pair d'entropie ($\eta,\phi$), alors le système est symétrisable \footnote{$\forall u \in \Omega,\, \exists A_0(u)\in \mathcal{M}_{M\times M}$ telle que $A_0(u)\, A(u)$ est symétrique}
et qu'il existe une variable d'entropie $\nu(u):=\nabla \eta(u)$ en bijection avec $\mathbf{u}$ la solution entropique de \eqref{Formulation_Faible} (voir \ref{entropie_Annexes} pour plus de détails) dans le sens où l'on peut écrire le système comme $\mathbf{u(\nu)}$. \\
Il a aussi été prouvé (voir \cite{SNKružkov_1970}) que pour un système scalaire, il existe une unique solution entropique dans $L^\infty \cap BV$,  associée à une donnée initale $u_0$.
